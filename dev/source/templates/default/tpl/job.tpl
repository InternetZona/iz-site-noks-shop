...
{block name=page_content}
    <div class="row">
        <div class="container">
            ...
            {if $jobs = json_decode({tv name=jobs}, true)}
                <ul class="collapsible jobs">

                    {foreach $jobs as $job}
                        <li>
                            <div class="collapsible-header"><div class="h2">{$job.job}</div></div>
                            <div class="collapsible-body">{$job.text}</div>
                        </li>
                    {/foreach}
            ...
            {elseif {field name=id} == 3}
                <a href="#modal-order-project" class="button-action modal-trigger col s6 m3 l2" onclick="yaCounter49093180.reachGoal('proekt');">
                    ...
                    Заказать проект</a>
            {elseif {field name=id} == 4 || {field name=id} == 5 || {field name=id} == 6 || {field name=id} == 78 || {field name=id} == 307}
            ...
            {elseif {field name=id} == 80}
                ...
                <a href="#modal-order-visit" class="button-action modal-trigger col s8 m3 l2 margin20" onclick="yaCounter49093180.reachGoal('zamer');">
                    ...
                    Заказать замер</a>
            {/if}
            {if {field name=id} == 6}<div class="col l12"><a href="{$modx->makeUrl(363)}">Вакансии ООО "НОКС МЕБЕЛЬ</a></div>{/if}
        </div>
    </div>
{/block}